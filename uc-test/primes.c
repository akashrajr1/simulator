...
void sieve(pint p)
{
	...
	int i, q;
 	...
 	/* filter out multiples of p: m*p, 
 	 * where m = 30*k + rem_num[i] (other m is not neccesary)
 	 * but some m is still redundent
 	 * k = 0, 1, ...; i = 0, 1, ..., 7;
 	 * ofs[i] stores the increment of index of pbits[]
 	 * (from p/30 to (rem_num[i]*p)/30) 
 	 */
	for (i = 0; i < 8; i++) {
		q = rem_num[i] * p;
		b[i] = ~bit_pos[q % 30];
 	...
 	/* ofs[i] now stores step by step increment
 	 */
	for (q = ofs[1], i = 7; i; i--)
		ofs[i] -= ofs[i-1];
 	/* ofs[0] is the increment of index
 	 * (from m*p to (m+30)*p)
 	 */
